const api_key = '0fbd85e8a0b3024896150837b687ad83';
let cityInput = document.getElementById('city_input'),
    ...
    feelsVal = document.getElementById('feelsvall'),
    hourlyForecastCard = document.querySelector('.hourly-forecast'),
    hourlyForecastWindCard = document.querySelector('.hourly-forecast-wind'),
...
function getWeatherDetails(name, lat, lon, country, state) {
    ...
    fetch(WEATHER_API_URL)
        ...
        .then(data => {
            ...
            document.getElementById("visibilityval").innerHTML = `${visibility / 1000} km`;
    ...
    .then(data => {
        let hourlyForecast = data.list;
        hourlyForecastWindCard.innerHTML = '';
        ...
        for (let i = 0; i <= 7; i++) {
            ...
            if (hr == 0) hr = 12;
            if (hr > 12) hr = hr - 12;
            const windSpeed = hourlyForecast[i].wind.speed;
            hourlyForecastWindCard.innerHTML += `
                <div class="tdcard">
                    <p>${hr} ${a}</p>
                    <i class='bx bx-wind bx-md'></i>
    ...
    navigator.geolocation.getCurrentPosition(position => {
        ...
        fetch(REVERSE_GEOCODING_URL)
            ...
            .then(data => {
                ...
                getWeatherDetails(cityName, latitude, longitude, data[0].country);
            })
            .catch(() => {
        ...
        if (error.code === error.PERMISSION_DENIED) {
            alert('Please reset Geolocation permission');
        }
    });
}
